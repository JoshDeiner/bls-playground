...
def map_bls_data_with_ids(bls_response):
    ...
    if not response:
        raise HTTPException(status_code=404, detail="No response received from the API")
    ...
    for idx, data_point in enumerate(series_data.get("data", [])):
        series_data_entry = {
            # You can let the database handle the ID instead of using idx+1 if needed
        ...
        calculations_entry = {
            "series_data_id": idx
            + 1,  # Corresponds to series_data ID (or use actual DB-assigned ID)
